...
Он в URL-ХЕШЕ страницы — значит НЕ попадает в Referer, историю и логи статики при
открытии страницы. Но в API-запросах ниже токен идёт В ПУТИ ({token}), поэтому он
ДОХОДИТ до бэка и МОЖЕТ попасть в access-логи nginx/приложения — это нормально для
unguessable-ссылки (токен работает как bearer). Никакой авторизации: доступ контролирует
только знание токена. PII на выдаче минимизирован (имя + компания).
"""
