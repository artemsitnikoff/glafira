...
# ВАЖНО: всю инструкцию кладём в USER-промпт (а не --append-system-prompt) — как ArkadyJarvis.
# Через системный промпт Claude Code модель не включает веб-поиск всерьёз и возвращает пусто.
_OSINT_PROMPT_TMPL = """Ты — AI-разведчик для рекрутера. Найди в открытом интернете публичную активность человека и верни структурированный результат. Активно используй инструменты WebSearch и WebFetch — сделай НЕСКОЛЬКО разных заходов (ФИО+компания, ФИО+город, по email, по нику из email, по номеру телефона, по конкретным платформам) и только ПОТОМ делай вывод. Не сдавайся после одного запроса.

Данные о человеке (по его согласию — используй ВСЁ как ключи поиска):
{query}

Что искать:
- Профили: GitHub, Habr / Habr Career, Stack Overflow / Stack Exchange, Telegram-каналы (t.me / TGStat), vc.ru, ВКонтакте, LinkedIn, личный сайт/блог.
- Упоминания: доклады на конференциях, статьи, интервью, новости — короткая цитата + ссылка.

Правила:
- Включай находку, если по контексту (ФИО + город / компания / email / тематика) это с ВЫСОКОЙ вероятностью тот же человек. При однофамильцах выбирай по совпадению компании / города / почты — не отбрасывай находку только из-за неполной 100% уверенности.
- У КАЖДОЙ находки обязательна реальная ссылка (URL), которую ты реально открыл/нашёл. Без URL — не добавляй.
- Числа в stats (звёзды, репозитории, подписчики, карма, репутация) бери ТОЛЬКО если реально видел их на странице; не выдумывай — иначе оставь stats пустым.
- Контакты (email/телефон) — это ключи поиска; в JSON их НЕ дублируй.

Верни ТОЛЬКО JSON, без markdown-обёртки и любого текста до/после:
{"profiles":[{"platform":"GitHub","handle":"@ник или /users/...","url":"https://...","stats":"412 ★ · 28 repo (или пусто)"}],"mentions":[{"quote":"короткая цитата","url":"https://..."}]}
Если после нескольких разных заходов реально ничего не нашёл — верни {"profiles":[],"mentions":[]}."""
...
async def _build_osint_blocks(candidate: Candidate) -> list[dict]:
    ...
    raw = await claude_cli_complete(
        ...
        system=None,  # инструкция в user-промпте (как ArkadyJarvis), НЕ в system
