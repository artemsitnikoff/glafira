...
async def list_saved_resume_searches(access_token: str, page: int = 0) -> dict:
    ...
    async with _get_client() as client:
        try:
            response = await client.get(
                ...
                headers={"Authorization": f"Bearer {access_token}"},
                params=[("page", str(page)), ("per_page", "10")],
            )
            if response.status_code == 403:
            ...
            # Логируем ТОЛЬКО на первой странице, чтобы не спамить на каждой странице пагинации.
            if page == 0:
