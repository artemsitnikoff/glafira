...
## 5. Запуск БД + миграции + seed (порядок важен)
...
## 7. HTTPS + nginx (ОБЩИЙ nginx — отдельный шаг)
...
## 8. Pre-flight чек-лист (перед первым заходом в браузер)
...
## 9. Smoke в браузере (после деплоя — отдельный чек-лист)
...
## 10. Известные пост-деплой долги (не блокеры)
