...
Завершён по основной функциональности. **~233 эндпоинта** в 25 роутерах (`app/api/v1/`), тесты в `tests/` (~80 файлов, 1103 теста, offline — LLM-ключ пуст, живые вызовы не идут). (Числа растут с каждой фичей — при сомнении пересчитать `grep`, не верить цифре вслепую. Последнее обновление CLAUDE.md — 2026-07-03, версия 0.9.205.)
...
**Текущая версия:** `frontend/src/lib/version.ts` (`APP_VERSION`) — на 2026-07-03 = **0.9.205**. Видна в Sidebar под «Глафира» (и в шапке суперадминки). **Bump на каждый значимый деплой.**
