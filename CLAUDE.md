...
Завершён по основной функциональности. **~194 эндпоинта** в 23 роутерах (`app/api/v1/`), тесты в `tests/` (offline — LLM-ключ пуст, живые вызовы не идут). (Числа растут с каждой фичей — при сомнении пересчитать `grep`, не верить цифре вслепую. Последнее обновление CLAUDE.md — 2026-06-14, версия 0.9.48.)
...
**Текущая версия:** `frontend/src/lib/version.ts` (`APP_VERSION`) — на 2026-06-14 = **0.9.48**. Видна в Sidebar под «Глафира» (и в шапке суперадминки). **Bump на каждый значимый деплой.**
