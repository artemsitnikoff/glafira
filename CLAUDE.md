...
**Субагентам ЗАПРЕЩЕНЫ любые git-операции** (add/commit/push/checkout/restore/reset) — они ТОЛЬКО редактируют файлы; коммит/пуш ведёт оркестратор после ревью диффа. Запрет в промпте субагенты ИГНОРИРУЮТ (повторно пушили в `main`, бампали `version.ts`, заводили вторую alembic-голову, тащили PII в публичный репо) — **единственная надёжная защита = ПОСТ-ФАКТУМ проверка оркестратором** после КАЖДОГО субагента: `git status`/`git log` (не пушил ли?), `gh run list` (деплой не упал?), одна alembic-голова?, `grep` фейков/`console`/выдуманных токенов/локальных импортов, прогон `tsc`/`build`/`py_compile` самому, чтение диффа на утечку `company_id`/PII, финальный `code-reviewer`. Память `subagents-no-git-operations`.
...
Завершён по основной функциональности. **~194 эндпоинта** в 23 роутерах (`app/api/v1/`), тесты в `tests/` (offline — LLM-ключ пуст, живые вызовы не идут). (Числа растут с каждой фичей — при сомнении пересчитать `grep`, не верить цифре вслепую. Последнее обновление CLAUDE.md — 2026-06-13, версия 0.9.29.)
...
**Текущая версия:** `frontend/src/lib/version.ts` (`APP_VERSION`) — на 2026-06-13 = **0.9.30**. Видна в Sidebar под «Глафира». **Bump на каждый значимый деплой.**
