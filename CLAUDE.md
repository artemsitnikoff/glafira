...
Завершён по основной функциональности. **~185+ эндпоинтов** в 23 роутерах (`app/api/v1/`; добавлены `candidate_import` + `/smart/base/*`), тесты в `tests/` (offline — LLM-ключ пуст, живые вызовы не идут). (Числа растут с каждой фичей — при сомнении пересчитать `grep`, не верить цифре вслепую. Последнее обновление CLAUDE.md — 2026-06-10, версия 0.7.1.)
...
- **Ветка Б — по СВОЕЙ базе** (`services/base_search.py`, с v0.7.0): чистый **SQL-поиск по `candidates`** + тонкий LLM-парс промта в фильтры. ⚠️ **БЕЗ векторов/pgvector/эмбеддингов/rerank** (семантика — отдельный заход B, ещё НЕ сделан; образ БД не трогать). `parse_query_to_criteria` (call_json → {role,skills,city,salary} + фолбэк на ключевые слова при сбое, НЕ 500); `search_base` (ILIKE last_position/city + EXISTS candidate_skills scoped-company + ЗП BETWEEN с NULL-инклюзив; **`match_percent` = РЕАЛЬНЫЙ overlap навыков**, не AI/cosine; сорт «больше совпало → выше» → ai_score → дата; кэп 100); `search_by_vacancy` (переиспользует `derive_vacancy_filters`, принимает override-критерии от фронта). Эндпоинты `/smart/base/{search,runs,count,runs/{id}/mark-added}` (RBAC manager-forbidden, синхронно, без фон-джоба). История `base_search_runs`. Опыт хранится строками → НЕ хард-SQL-фильтр (мягкий). «+ В вакансию» переиспользует `assign_candidate_to_vacancy`. Первый реальный поиск на живой базе — заказчик.
...
**Свежие фичи (0.7.x) — УМНЫЙ ПОДБОР ПО СВОЕЙ БАЗЕ:** 0.7.0 — развилка источника в `/smart` + ветка Б «по своей базе» (SQL-поиск + LLM-парс промта, БЕЗ векторов — см. §3); 0.7.1 — автофильтры vacancy-метода ветки Б приведены к стилю фильтров кандидатов (Должность/Город/Опыт/ЗП/Навыки), город/ЗП реально фильтруют, правки влияют на поиск. Заход B (семантика/вектора/rerank) — ОТЛОЖЕН. Память `smart-search-state`.

**Текущая версия:** `frontend/src/lib/version.ts` (`APP_VERSION`) — на 2026-06-10 = **0.7.1**. Видна в Sidebar под «Глафира». **Bump на каждый значимый деплой.**
