...
Завершён по основной функциональности. **~233 эндпоинта** в 25 роутерах (`app/api/v1/`), тесты в `tests/` (~80 файлов, 1103 теста, offline — LLM-ключ пуст, живые вызовы не идут). (Числа растут с каждой фичей — при сомнении пересчитать `grep`, не верить цифре вслепую. Последнее обновление CLAUDE.md — 2026-06-22, версия 0.9.146.)

> ✅ **Полный pytest зелёный: 1103/1103** (~80 файлов, прогон на VPS 2026-06-22). Два раунда чистки тест-долга: раунд 1 (713, v0.9.25-29 — 90 падений→0, 9 реальных прод-багов); раунд 2 (1103, v0.9.144 — после security-эры 0.9.127-143 всплыло 24 падения, триаж 12-агентным Workflow → ВСЕ протухшие тесты, 0 реальных багов, прод-код не тронут). **Урок: гонять ВЕСЬ suite после рефактор/security-эр** (кросс-файловые регрессы + протухшие тесты) + при починке падающего теста сверять ВЕСЬ его body (httpx `.json()`/`.status_code` СИНХРОННЫ → `MagicMock`, не `AsyncMock` — вечная грабля). Память `full-suite-test-debt`.
...
**Текущая версия:** `frontend/src/lib/version.ts` (`APP_VERSION`) — на 2026-06-22 = **0.9.146**. Видна в Sidebar под «Глафира» (и в шапке суперадминки). **Bump на каждый значимый деплой.**
